...
const { SCIENCE_PAGE_LINK } = process.env;

const SciencePageController = async (req: Request, res: Response) => {
  try {
    const response = await fetch(SCIENCE_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.scienceNewsPost.createMany({
      data: items,
    });
...
export default SciencePageController;
