...
const { FASHION_PAGE_LINK } = process.env;

const FashionPageController = async (req: Request, res: Response) => {
  try {
    // console.log("india-site", JSON.parse(INDIA_PAGE_LINK));
    const response = await fetch(FASHION_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.fashionNewsPost.createMany({
      data: items,
    });
...
export default FashionPageController;
