...
const { BUSINESS_PAGE_LINK } = process.env;

const BusinessPageController = async (req: Request, res: Response) => {
  try {
    // console.log("india-site", JSON.parse(INDIA_PAGE_LINK));
    const response = await fetch(BUSINESS_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.businessNewsPost.createMany({
      data: items,
    });
