...
const { HEALTH_AND_FITNESS_PAGE_LINK } = process.env;

const healthPageController = async (req: Request, res: Response) => {
  try {
    const response = await fetch(HEALTH_AND_FITNESS_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.healthAndFitnessNewsPost.createMany({
      data: items,
    });
...
export default healthPageController;
