...
const { EDUCATION_PAGE_LINK } = process.env;

const EducationPageController = async (req: Request, res: Response) => {
  try {
    // console.log("india-site", JSON.parse(INDIA_PAGE_LINK));
    const response = await fetch(EDUCATION_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.educationNewsPost.createMany({
      data: items,
    });
