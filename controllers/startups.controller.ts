...
const { STARTUPS_PAGE_LINK } = process.env;

const StartUpsPageController = async (req: Request, res: Response) => {
  try {
    const response = await fetch(STARTUPS_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.startupsNewsPost.createMany({
      data: items,
    });
...
export default StartUpsPageController;
