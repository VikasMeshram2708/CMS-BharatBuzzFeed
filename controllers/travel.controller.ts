...
const { TRAVEL_PAGE_LINK } = process.env;

const TravelPageController = async (req: Request, res: Response) => {
  try {
    const response = await fetch(TRAVEL_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.travelNewsPost.createMany({
      data: items,
    });
...
export default TravelPageController;
