...
const { ENTERTAINMENT_PAGE_LINK } = process.env;

const EntertainmentPageController = async (req: Request, res: Response) => {
  try {
    const response = await fetch(ENTERTAINMENT_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.entertainmentNewsPost.createMany({
      data: items,
    });
...
export default EntertainmentPageController;
