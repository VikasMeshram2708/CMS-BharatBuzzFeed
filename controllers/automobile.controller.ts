...
const { AUTOMOBILE_PAGE_LINK } = process.env;

const AutomobilePageController = async (req: Request, res: Response) => {
  try {
    // console.log("india-site", JSON.parse(INDIA_PAGE_LINK));
    const response = await fetch(AUTOMOBILE_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.automobileNewsPost.createMany({
      data: items,
    });
...
export default AutomobilePageController;
