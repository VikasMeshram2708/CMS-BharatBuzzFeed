...
const { MISCELLANEOUS_PAGE_LINK } = process.env;

const MiscellaneousPageController = async (req: Request, res: Response) => {
  try {
    const response = await fetch(MISCELLANEOUS_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.miscellaneousNewsPost.createMany({
      data: items,
    });
...
export default MiscellaneousPageController;
