...
const { INTERNATIONAL_PAGE_LINK } = process.env;

const InternationalPageController = async (req: Request, res: Response) => {
  try {
    // console.log("india-site", JSON.parse(INDIA_PAGE_LINK));
    const response = await fetch(INTERNATIONAL_PAGE_LINK!);
    ...
    await connectToDb();
    // save to DB
    await prisma.internationalNewsPost.createMany({
      data: items,
    });
...
export default InternationalPageController;
